...
namespace gfx
{
    ...
    void imgui::apply_theme() const
    {
        ...
        // Default Font
        std::experimental::filesystem::path res = SOURCE_DIRECTORY "/global/res/";
        if (exists(res / "ui/fonts/Ruda-Bold.ttf"))
            res = res / "ui/fonts/Ruda-Bold.ttf";
        else if (exists(std::experimental::filesystem::path("../res/ui/fonts/Ruda-Bold.ttf")))
            res = "../res/ui/fonts/Ruda-Bold.ttf";
        else
            std::cerr << "IMGUI: Font not found!";

        atlas->AddFontFromFileTTF(res.string().c_str(), 12);
        atlas->AddFontFromFileTTF(res.string().c_str(), 10);
        atlas->AddFontFromFileTTF(res.string().c_str(), 14);
        atlas->AddFontFromFileTTF(res.string().c_str(), 18);
