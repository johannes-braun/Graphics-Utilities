...
namespace gfx
{
    void handle_node(scene_file& file, aiNode* node, const aiScene* scene, const glm::mat4& transform) noexcept
    {
        ...
        {
            file.meshes.at(scene->mMeshes[node->mMeshes[i]]->mName.C_Str()).transform = static_cast<res::transform>(node_trafo);
        }
        ...
        : file(path)
    {
        ...
        for (uint32_t i = 0; i < scene->mNumMaterials; ++i)
        {
            ...
            materials.emplace(name.C_Str(), current_material);
        }
...
#pragma omp parallel for schedule(static)
            ...
            {
                current_mesh.vertices[i] = res::vertex(
            ...
            current_mesh.material = &(materials.at(name.C_Str()));
            meshes.emplace(ai_mesh->mName.C_Str(), current_mesh);
        }
