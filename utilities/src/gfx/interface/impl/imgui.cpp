...
namespace gfx
{
        ...
        : _window(w)
    {
        ...
        io.SetClipboardTextFn = [](void* data, const char* text) { glfwSetClipboardString(*(reinterpret_cast<imgui*>(data)->_window), text); };
        io.GetClipboardTextFn = [](void* data) { return glfwGetClipboardString(*(reinterpret_cast<imgui*>(data)->_window)); };
        
        _key_callback = _window->key_callback.add([this](GLFWwindow*, int key, int, int action, int mods)
        {
            auto&& io = _context->IO;
        ...
        _mousebutton_callback = _window->mouse_button_callback.add([this](GLFWwindow*, int button, int action, int mods)
        {
            auto&& io = _context->IO;
        ...
        _scroll_callback = _window->scroll_callback.add([this](GLFWwindow*, double x, double y)
        {
            _mouse_wheel_delta += float(y);
        });

        _char_callback = _window->char_callback.add([this](GLFWwindow*, uint32_t c)
        {
            auto&& io = _context->IO;
            if (c > 0 && c < 0x10000)
    ...
    imgui::~imgui()
    {
        _window->key_callback.remove(_key_callback);
        _window->char_callback.remove(_char_callback);
        _window->mouse_button_callback.remove(_mousebutton_callback);
        _window->scroll_callback.remove(_scroll_callback);
    }
