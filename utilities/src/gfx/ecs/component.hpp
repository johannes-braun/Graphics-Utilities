...
struct component_base
{
    ...
    static decltype(auto) get_creator(id_t id) { return std::get<component_creator_fun>(_types[static_cast<size_t>(id)]); }
    static decltype(auto) get_deleter(id_t id) { return std::get<component_deleter_fun>(_types[static_cast<size_t>(id)]); }
    static decltype(auto) type_size(id_t id) { return std::get<size_t>(_types[static_cast<size_t>(id)]); }
...
template <typename T> struct component : component_base
{
    ...
    static const id_t                  id      = register_type(create<type>, destroy<type>, size);
    static const component_creator_fun creator = create<type>;
    static const component_deleter_fun deleter = destroy<type>;
};
} // namespace gfx::ecs