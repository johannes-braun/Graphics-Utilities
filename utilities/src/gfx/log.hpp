...
static inline std::string expand_label(logger_label l)
{
        ...
        class log_printer
        {
            ...
            template <typename Stream> void print_one(Stream& stream, std::size_t index)
            {
                if(_cstate == state::style && enable_styling_v<Stream>)
                    logging_thread().enqueue([str = _outputs[index].str(), &strm = stream]() { strm << str << 'm' << "\n"; });
                else
                    logging_thread().enqueue([str = _outputs[index].str(), &strm = stream]() { strm << str << "\n"; });
            }

            template <typename Tuple, std::size_t... Indices> void print_each(Tuple&& tuple, std::index_sequence<Indices...>)
            {
                std::unique_lock lock(logging_thread().mtx);
...
struct highlighter
{
    ...
    {
        l << style::bold << color_fg(230) << color_bg(_color) << (s.empty() ? _label : s) << color_bg(251) << color_fg(251) << ':';
    }
...
inline std::ofstream log_err{"errors.log"};
inline std::ofstream  log_all{"output.log"};

template <color_bg C> log_impl::logger clogc{highlighter(expand_label(log_label("user")), uint32_t(C)), std::cout};

inline static log_impl::logger clogd{highlighter(expand_label(log_label("debug")), 31), std::cout};
inline static log_impl::logger clogh{highlighter(expand_label(log_label("hint")), 59), std::cout};
inline static log_impl::logger clogi{highlighter(expand_label(log_label("info")), 35), log_all, std::cout};
inline static log_impl::logger clogw{highlighter(expand_label(log_label("warning")), 172), std::cout, log_all};
inline static log_impl::logger cloge{highlighter(expand_label(log_label("error")), 124), std::cout, log_all, log_err};
} // namespace gfx