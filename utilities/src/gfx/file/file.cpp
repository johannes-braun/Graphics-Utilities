...
namespace gfx
{
    ...
    std::optional<file> file::open_dialog(const std::string& title, const files::path& default_path, const std::vector<std::string>& filters, const std::string& filter_description)
    {
        std::vector<const char*> f(filters.size());
        for (int i=0; i<f.size(); ++i) f[i] = filters[i].c_str();
        const char* path = tinyfd_openFileDialog(title.c_str(), default_path.string().c_str(), int(f.size()), f.data(), filter_description.c_str(), false);
        return path ? file(path) : std::optional<file>();
    }
    ...
    std::vector<file> file::open_dialog_multi(const std::string& title, const files::path& default_path, const std::vector<std::string>& filters)
    {
        std::vector<const char*> f(filters.size());
        for (int i=0; i<f.size(); ++i) f[i] = filters[i].c_str();
    ...
    std::vector<file> file::open_dialog_multi(const std::string& title, const files::path& default_path, const std::vector<std::string>& filters, const std::string& filter_description)
    {
        std::vector<const char*> f(filters.size());
        for (int i=0; i<f.size(); ++i) f[i] = filters[i].c_str();
        const char* path = tinyfd_openFileDialog(title.c_str(), default_path.string().c_str(), int(f.size()), f.data(), filter_description.c_str(), true);
        return separate(path);
    }
    ...
    std::optional<file> file::save_dialog(const std::string& title, const files::path& default_path, const std::vector<std::string>& filters, const std::string& filter_description)
    {
        std::vector<const char*> f(filters.size());
        for (int i=0; i<f.size(); ++i) f[i] = filters[i].c_str();
        const char* path = tinyfd_saveFileDialog(title.c_str(), default_path.string().c_str(), int(f.size()), f.data(), filter_description.c_str());
        if (path)
        {
            file result;
            result.path = path;
