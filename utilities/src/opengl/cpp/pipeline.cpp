...
namespace gl
{
std::unordered_set<pipeline*> pipeline::_all_pipelines;
std::mutex                    pipeline::_all_pipelines_mutex;
...
stage::operator const std::shared_ptr<shader>&() const { return _shader; }

shader* stage::operator->() const { return _shader.get(); }

shader* stage::get() const { return _shader.get(); }

stage::operator bool() const noexcept { return _shader != nullptr; }

pipeline::pipeline() noexcept
{
    {
        std::unique_lock<std::mutex> lock(_all_pipelines_mutex);
        _all_pipelines.emplace(this);
    }
...
const std::shared_ptr<shader>& pipeline::at(GLenum stage) const { return _stages.at(stage); }

stage& pipeline::operator[](GLenum stage) { return at(stage); }

const std::shared_ptr<shader>& pipeline::operator[](GLenum stage) const { return at(stage); }
...
pipeline::operator mygl::pipeline() const noexcept { return _id; }

void pipeline::reload_all()
{
    std::unique_lock<std::mutex> lock(_all_pipelines_mutex);
    for(auto&& pp : _all_pipelines)
        pp->reload();
}
...
void pipeline::draw_instanced(GLenum primitive, size_t count, size_t instances, size_t first_vertex,
                              size_t first_instance) const noexcept
{
    glDrawArraysInstancedBaseInstance(
            primitive, int(first_vertex), int(count), int(instances), uint32_t(first_instance));
}
...
void compute_pipeline::dispatch(uint32_t count_x, uint32_t count_y, uint32_t count_z) noexcept
{
    ...
    glBindProgramPipeline(_id);
    glDispatchCompute(invocation_count(count_x, _group_sizes[0]),
                      invocation_count(count_y, _group_sizes[1]),
                      invocation_count(count_z, _group_sizes[2]));
}

compute_pipeline::operator mygl::pipeline() const noexcept { return _id; }

const glm::ivec3& compute_pipeline::group_sizes() const noexcept { return _group_sizes; }

const std::shared_ptr<shader>& compute_pipeline::stage() const noexcept { return _shader; }

shader* compute_pipeline::operator->() const noexcept { return _shader.get(); }
