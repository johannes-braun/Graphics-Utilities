...
public:
    ...
    operator mygl::renderbuffer() const noexcept;

    int width() const noexcept;
    int height() const noexcept;

private:
    mygl::renderbuffer _id = mygl::renderbuffer::zero;
    int                _width{1};
    int                _height{1};
...
public:
    friend framebuffer;
    using all_types =
            std::variant<std::nullptr_t, std::shared_ptr<texture>, std::shared_ptr<renderbuffer>>;
...
private:
    ...
    mygl::framebuffer _framebuffer     = mygl::framebuffer::zero;
...
public:
    ...
    const attachment& operator[](GLenum type) const;
                      operator mygl::framebuffer() const noexcept;
    ...
    using value_type_check_t =
            std::enable_if_t<std::is_same_v<typename base_t<T>::value_type, float>>;
    template <typename T, typename = std::void_t<value_type_check_t<T, float>,
                                                 decltype(std::declval<base_t<T>>()[0])>>
    ...
    template <typename Container,
              typename = std::void_t<
                      ...
                      std::enable_if_t<!std::is_same_v<Container, std::initializer_list<GLenum>> &&
                                       std::is_same_v<typename Container::value_type, GLenum>>>>
    ...
    void blit(const framebuffer& other, int x, int y, int width, int height, GLbitfield mask,
              GLenum filter = GL_NEAREST) const noexcept;
    void blit(const framebuffer& other, int src_x, int src_y, int src_width, int src_height,
              int dst_x, int dst_y, int dst_width, int dst_height, GLbitfield mask,
              GLenum filter = GL_NEAREST) const noexcept;

private:
    void status_assert() const noexcept;

    mygl::framebuffer            _id = mygl::framebuffer::zero;
    GLenum                       _readbuffer;
