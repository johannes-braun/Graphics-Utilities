...
uniform float clip_near = 0.1f;
uniform float clip_far = 1000.f;
uniform int max_march_steps = 200;
...
float map(vec3 pos)
{
    ...
    res = opS(sdTorus(pos + vec3(0, -1.5f, 1.f), vec2(1.5f, 0.3f)), res);
...
void main()
{
    ...
    vec2 scaled_uv = uv * 2 - 1;
    scaled_uv.x *= aspect;
    vec3 dir = mat3(inv_view_mat) * normalize(vec3(scaled_uv, -1.5));
