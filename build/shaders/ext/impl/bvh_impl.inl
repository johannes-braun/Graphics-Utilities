...
#define BVH_IMPL_STACKLESS
#ifdef BVH_IMPL_STACKLESS
...
{
    bvh_result result;
    result.near_distance = max_distance; // = inf
    result.hits = false;
    ...
    while(hits_scene)
    {
        while(current_node->type == bvh_node_type_inner)
        {
            ...
            bool hits_left = intersect_bounds(origin, direction, left->bounds_min.xyz, left->bounds_max.xyz, result.near_distance, min_left);
            bool hits_right = intersect_bounds(origin, direction, right->bounds_min.xyz, right->bounds_max.xyz, result.near_distance, min_right);
    ...
    const float max_distance)
{
    ...
    int stack[20];
    #define push_stack(num) (stack[++stack_ptr] = num)
    #define pop_stack() (stack[stack_ptr--])
    int stack_ptr = -1;
    ...
    while(hits_scene)
    {
        while(current_node->type == bvh_node_type_inner)
        {
            ...
            {
                // Push to stack for later work.
                // Only if needed.
                push_stack((int)(second - root_node));
            }
        ...
        current_node = root_node + pop_stack();
    }
