...
namespace nana{	namespace drawerbase
{
	namespace button
	{
		...
		void trigger::_m_draw_background(graph_reference graph)
		{
			...
			auto from = attr_.bgcolor.blend(colors::button_face, 0);
			auto to = attr_.bgcolor.blend(colors::button_face, 0);

			if (element_state::pressed == attr_.e_state)
			{
				r.x = r.y = 2;
				std::swap(from, to);
                from = to = attr_.bgcolor.blend(colors::dark_gray, 0.5f);
			}
		...
		void trigger::_m_draw_border(graph_reference graph)
		{
			...
			::nana::color lt(colors::gray_border), rb(colors::gray_border);
			...
			graph.palette(false, colors::gray_border);

			draw.corner(r.pare_off(1), 1);

			/*if (element_state::pressed == attr_.e_state)
				graph.rectangle(r, false, colors::gray_border);*/
		}
