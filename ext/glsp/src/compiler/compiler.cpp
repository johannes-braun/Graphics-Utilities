...
compiled_shader load_opengl_binary(const files::path& file, uint32_t type, const std::vector<files::path>& includes,
                                   const std::vector<definition>& definitions, const std::string& prefix, const std::string& postfix)
{
    ...
    const char* sources[3] = {prefix.c_str(), processed.contents.c_str(), postfix.c_str()};
...
void compiler::set_cache_dir(const glsp::files::path& dir) { _cache_dir = dir; }

void compiler::set_default_prefix(const std::string& prefix) { _default_prefix = prefix; }

void compiler::set_default_postfix(const std::string& postfix) { _default_postfix = postfix; }
...
shader_binary compiler::compile(const glsp::files::path& shader, format format, bool force_reload, std::vector<glsp::files::path> includes,
                                std::vector<glsp::definition> definitions)
{
    ...
    if(reload)
    {
        ...
        {
            compiled_shader compiled = load_opengl_binary(
                    shader, type_from_extension(shader.extension()), includes, definitions, _default_prefix, _default_postfix);
