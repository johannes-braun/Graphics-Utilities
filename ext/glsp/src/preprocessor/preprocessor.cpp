...
void process_impl(const files::path& file_path, const std::string& contents, const std::vector<files::path>& include_directories,
                  processed_file& processed, std::set<files::path>& unique_includes, std::stringstream& result)
{
    ...
    while(!cls::is_eof(text_ptr))
    {
        ...
        else if(cls::is_directive(text_ptr, current_line != 1))
        {
            ...
            if(cls::is_token_equal(directive_name, "version", 7))
            {
                ...
                processed.version = (*text_ptr - '0') * 100 + (*(text_ptr + 1) - '0') * 10 + (*(text_ptr + 2) - '0');
                ...
                else
                {
                    ...
                    syntax_error_print(
                            current_file,
                            current_line,
                            strfmt(strings::serr_unrecognized_profile, std::string(text_ptr, skip::to_endline(text_ptr)).c_str()));
                    processed.definitions["GL_core_profile"] = 1;
                    processed.profile                        = shader_profile::core;
                }
                ...
                if(extension == "all")
                {
                    ...
                    else
                    {
                        ...
                        syntax_error_print(
                                current_file,
                                current_line,
                                strfmt(strings::serr_extension_all_behavior, std::string(text_ptr, skip::to_endline(text_ptr)).c_str()));
                    }
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        syntax_error_print(
                                current_file,
                                current_line,
                                strfmt(strings::serr_extension_behavior, std::string(text_ptr, skip::to_endline(text_ptr)).c_str()));
                    }
                }
                while(!cls::is_newline(text_ptr))
                    result << *text_ptr++;
            ...
            else if(const auto elif = cls::is_token_equal(directive_name, "elif", 4);
                    cls::is_token_equal(directive_name, "if", 2, true, false) || (elif))
            {
                    ...
                    for(;; ++text_ptr)
                    {
                        ...
                        else if(const auto space_skipped = skip::space(text_ptr); (*(text_ptr - 1) == '\n') && *space_skipped == '#')
                        {
                            ...
                            if(memcmp(skip::space(text_ptr + 1), "if", 2) == 0)
                            {
                                ...
                                while(!(cls::is_directive(deeper_skipped) &&
                                        (cls::is_token_equal(skip::space(deeper_skipped + 1), "endif", 5) ||
                                         cls::is_token_equal(deeper_skipped + 1, "elif", 4))))
                                {
                                    text_ptr = skip::over_comments(text_ptr, current_file, current_line, processed, result);
                                    if(cls::is_newline(text_ptr))
                                        ctrl::increment_line(current_line, processed);
            ...
            else if(cls::is_token_equal(directive_name, "include", 7))
            {
                ...
                if((include_filename.front() != '\"' && include_filename.back() != '\"') &&
                   (include_filename.front() != '<' && include_filename.back() != '>'))
                {
                    ++processed.error_count;
                ...
                {
                    ++processed.error_count;
                    syntax_error_print(current_file,
                                       current_line,
                                       strfmt(strings::serr_file_not_found,
                                              std::string(include_filename.begin() + 1, include_filename.end() - 1).c_str()));
                    return;
                }
...
{
   /* auto full_path = file_path;
    for(const auto& inc : include_directories)
        if(exists(inc / file_path))
        {
            full_path = inc / file_path;
            break;
        }
*/
    if(!exists(file_path))
...
processed_file preprocess_source(const std::string& source, const std::string& name, const std::vector<files::path>& include_directories,
                                 const std::vector<definition>& definitions)
{
    ...
    if(!gl_initialized || !lgl::valid())
    {
        lgl::reload();
        if(!glGetIntegerv)
            glGetIntegerv = reinterpret_cast<decltype(glGetIntegerv)>(lgl::load_function("glGetIntegerv"));
        if(!glGetStringi)
            glGetStringi = reinterpret_cast<decltype(glGetStringi)>(lgl::load_function("glGetStringi"));
...
void state::add_definition(const definition& d) { _definitions.push_back(d); }

void state::remove_definition(const std::string& name)
{
    if(const auto it = std::find_if(_definitions.begin(), _definitions.end(), [&name](const definition& def) { return def.name == name; });
       it != _definitions.end())
        _definitions.erase(it);
}

void state::add_include_dir(const files::path& dir) { _include_directories.push_back(dir); }
...
processed_file state::preprocess_file(const files::path& file_path, std::vector<files::path> include_directories,
...
processed_file state::preprocess_source(const std::string& source, const std::string& name, std::vector<files::path> include_directories,
                                        std::vector<definition> definitions)
{
    include_directories.insert(include_directories.begin(), _include_directories.begin(), _include_directories.end());
    definitions.insert(definitions.begin(), _definitions.begin(), _definitions.end());
...
bool processed_file::valid() const noexcept { return error_count == 0; }

processed_file::operator bool() const noexcept { return valid(); }
}
