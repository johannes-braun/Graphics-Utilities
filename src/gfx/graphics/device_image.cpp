...
const uint32_t& device_image::dimensions() const noexcept { return _layer_dimensions; }

const format& device_image::pixel_format() const noexcept { return _format; }

const extent& device_image::extents() const noexcept { return _extent; }

const uint32_t& device_image::levels() const noexcept { return _levels; }
...
device_image::device_image(img_type type, format format, const extent& size, uint32_t levels)
        ...
        , _layer_dimensions(static_cast<uint32_t>(type))
        , _format(format)
        , _extent(size)
        , _levels(levels)
        , _samples(sample_count::x1)
{
