...
template <typename T> buffer<T>& buffer<T>::operator=(const buffer& other) noexcept
{
    _usage          = other._usage;
    _size           = other._size;
...
template <typename T> buffer<T>& buffer<T>::operator=(buffer&& other) noexcept
{
    _id             = other._id;
    _data           = other._data;
...
template <typename T> buffer<T>::~buffer() noexcept
{
    if(_id != mygl::buffer::zero)
        glDeleteBuffers(1, &_id);
}
...
template <typename T>
buffer<T>::buffer(std::vector<T> list, GLbitfield usage)
        : buffer(list.begin(), list.end(), usage)
{
}
...
template <typename T>
template<typename>
buffer<T>::buffer(const T& item, GLbitfield usage)
        : buffer({item}, usage)
{
...
template <typename T>
template <size_t L>
buffer<T>::buffer(std::array<T, L> list, GLbitfield usage)
        : buffer(list.begin(), list.end(), usage)
{
}
...
template <typename T> void buffer<T>::push_back(T&& value)
{
    const size_t     last_size          = _size;
    const bool       was_data_full_size = _data_full_size;
    const GLbitfield map_access         = _data_access;
    ...
    {
        reserve(static_cast<size_t>(std::max(1.0, static_cast<double>(_reserved_size)) *
                                    compute_size_increase()));
    }
...
template <typename T> void buffer<T>::push_back(const T& value)
{
    const size_t     last_size          = _size;
    const bool       was_data_full_size = _data_full_size;
    const GLbitfield map_access         = _data_access;
...
template <typename T> template <typename... As> void buffer<T>::emplace_back(As&&... value)
{
        ...
        if constexpr(iterator_has_difference_v<It>)
        {
            ...
            if(new_size > reserved)
            {
                while(new_size > reserved)
                    reserved = static_cast<size_t>(std::max(1.0, static_cast<double>(reserved)) *
                                                   compute_size_increase());
                reserve(reserved);
            }
            ...
            if(_size > 1 && (std::distance(&*begin, &*std::prev(end)) != dist - 1))
            {
                ...
                else
                {
                    ...
                    glNamedBufferSubData(
                            _id, insert_position * sizeof(T), dist * sizeof(T), temp.data());
                }
            }
            else
            {
...
template <typename T> inline typename buffer<T>::iterator buffer<T>::erase(iterator it)
{
...
template <typename T> double buffer<T>::compute_size_increase() const noexcept
{
    return 1.0 + (10.0 / std::max(1.0, std::pow(_size, 0.35)));
}

template <typename T> void buffer<T>::shrink_to_fit()
{
    const bool       was_data_full_size = _data_full_size;
    const GLbitfield map_access         = _data_access;
    if(was_data_full_size && _size != 0)
...
template <typename T> inline void buffer<T>::bind(GLenum target, uint32_t binding) const noexcept
{
    bind(target, binding, 0, _size);
}

template <typename T>
inline void buffer<T>::bind(GLenum target, uint32_t binding, ptrdiff_t offset, size_t size) const
        noexcept
{
    glBindBufferRange(target, binding, _id, offset * sizeof(T), size * sizeof(T));
}

template <typename T> inline bool buffer<T>::empty() const noexcept { return _size == 0; }

template <typename T> inline size_t buffer<T>::size() const noexcept { return _size; }

template <typename T> inline size_t buffer<T>::capacity() const noexcept { return _reserved_size; }

template <typename T> void buffer<T>::clear() noexcept
{
    const bool       was_data_full_size = _data_full_size;
    const GLbitfield map_access         = _data_access;
    if(was_data_full_size && _size != 0)
...
template <typename T> void buffer<T>::clear(const T& value) noexcept
{
    const bool       was_data_full_size = _data_full_size;
    const GLbitfield map_access         = _data_access;
    if(was_data_full_size && _size != 0)
...
template <typename T> void buffer<T>::resize(size_t size, GLbitfield usage)
{
    resize(size, T(), usage);
}

template <typename T> void buffer<T>::resize(size_t size, const T& init, GLbitfield usage)
{
    if(usage != keep_usage)
        _usage = usage;
    reserve(size);
...
template <typename T> void buffer<T>::reserve(size_t size)
{
    if((_usage & GL_DYNAMIC_STORAGE_BIT) != GL_DYNAMIC_STORAGE_BIT)
        throw std::invalid_argument(
                "Cannot reserve or resize a buffer without GL_DYNAMIC_STORAGE_BIT set.");
    ...
    if(was_mapped)
    {
        ...
        if(_size > size)
        {
            map_size = static_cast<size_t>(std::max<ptrdiff_t>(std::min(static_cast<ptrdiff_t>(map_size), max_size), 0l));
        }
...
template <typename T> void buffer<T>::init_handle() noexcept
{
...
template <typename T> void buffer<T>::map(GLbitfield access)
{
    _data_full_size = true;
    map(_size, access);
}

template <typename T> void buffer<T>::map(size_t count, GLbitfield access)
{
    map(0, count, access);
}

template <typename T> bool buffer<T>::is_mapped() const noexcept
{
    return (_data_full_size && _size != 0) || _data != nullptr;
}

template <typename T> void buffer<T>::map(size_t begin, size_t count, GLbitfield access)
{
    ...
    if(count != 0)
        _data = static_cast<T*>(
                glMapNamedBufferRange(_id, begin * sizeof(T), count * sizeof(T), access));
    else
        _data = nullptr;
}

template <typename T> void buffer<T>::flush_element(size_t index) { flush_elements(index, 1); }

template <typename T> void buffer<T>::flush_elements(size_t begin, size_t count)
{
    flush_bytes(begin * sizeof(T), count * sizeof(T));
}

template <typename T> void buffer<T>::flush_bytes(size_t offset, size_t size)
{
    if(offset + size > _data_size * sizeof(T))
        throw std::out_of_range("Range out of bounds.");
    synchronize();
    glFlushMappedNamedBufferRange(_id, offset, size);
}

template <typename T> void buffer<T>::flush() { flush_elements(_data_offset, _data_size); }

template <typename T> void buffer<T>::unmap() noexcept
{
    synchronize();
...
template <typename T> void buffer<T>::synchronize() const noexcept
{
    for(int i = 0; i < _cached.size(); ++i)
    {
        if((_usage & GL_DYNAMIC_STORAGE_BIT) == GL_DYNAMIC_STORAGE_BIT && _cached[i].second)
            glNamedBufferSubData(
                    _id, _cached[i].first * sizeof(T), sizeof(T), _cached[i].second.get());
    }
}

template <typename T> inline uint64_t buffer<T>::handle() const noexcept { return _handle; }

template <typename T> T& buffer<T>::at(size_t index)
{
    ...
    if(!_data)
    {
        ...
        {
            if((_usage & GL_DYNAMIC_STORAGE_BIT) == GL_DYNAMIC_STORAGE_BIT &&
               _cached[_cached_index].second)
                glNamedBufferSubData(_id,
                                     _cached[_cached_index].first * sizeof(T),
                                     sizeof(T),
                                     _cached[_cached_index].second.get());
            if(!_cached[_cached_index].second)
                _cached[_cached_index].second =
                        std::unique_ptr<T>(static_cast<T*>(malloc(sizeof(T))));
            glGetNamedBufferSubData(
                    _id, index * sizeof(T), sizeof(T), _cached[_cached_index].second.get());
        }
        else
        {
            _cached_index = idx;
...
template <typename T> T& buffer<T>::operator[](size_t index) { return at(index); }

template <typename T> const T& buffer<T>::operator[](size_t index) const { return at(index); }

template <typename T> T* buffer<T>::data() noexcept { return _data; }

template <typename T> const T* buffer<T>::data() const noexcept { return _data; }
...
{
    if(_data)
        return bounded_buffer_data<T>(this, _data_size);
    else
        return bounded_buffer_data<T>(this, _size);
}

template <typename T> typename buffer<T>::iterator buffer<T>::begin() noexcept
{
    return iterate().begin();
}

template <typename T> typename buffer<T>::iterator buffer<T>::end() noexcept
{
    return iterate().end();
}

template <typename T> typename buffer<T>::const_iterator buffer<T>::begin() const noexcept
{
    return iterate().cbegin();
}

template <typename T> typename buffer<T>::const_iterator buffer<T>::end() const noexcept
{
    return iterate().cend();
}

template <typename T> typename buffer<T>::const_iterator buffer<T>::cbegin() const noexcept
{
    return iterate().cbegin();
}

template <typename T> typename buffer<T>::const_iterator buffer<T>::cend() const noexcept
{
    return iterate().cend();
}

template <typename T> typename buffer<T>::reverse_iterator buffer<T>::rbegin() noexcept
{
    return iterate().rbegin();
}

template <typename T> typename buffer<T>::reverse_iterator buffer<T>::rend() noexcept
{
    return iterate().rend();
}

template <typename T> typename buffer<T>::const_reverse_iterator buffer<T>::rbegin() const noexcept
{
    return iterate().crbegin();
}

template <typename T> typename buffer<T>::const_reverse_iterator buffer<T>::rend() const noexcept
{
    return iterate().crend();
}

template <typename T> typename buffer<T>::const_reverse_iterator buffer<T>::crbegin() const noexcept
{
    return iterate().crbegin();
}

template <typename T> typename buffer<T>::const_reverse_iterator buffer<T>::crend() const noexcept
{
    return iterate().crend();
}
}