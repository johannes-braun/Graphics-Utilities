    ...
    case 2:
    {
        ...
        if (glCreateTextures) {
            ...
            if (_type == GL_TEXTURE_2D_MULTISAMPLE_ARRAY)
                glTextureStorage3DMultisample(_handle, static_cast<int>(samples), _internal_format, _extent.width, _extent.height,
                                              _extent.depth, true);
            else
        ...
        else
        {
            ...
            if (_type == GL_TEXTURE_2D_MULTISAMPLE_ARRAY)
                glTexStorage3DMultisample(_type, static_cast<int>(samples), _internal_format, _extent.width, _extent.height, _extent.depth,
                                          true);
            else
...
{
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, handle_cast<mygl::buffer>(image.storage()));
    switch (_type)
    {
    case GL_TEXTURE_3D:
        if (glTextureSubImage3D)
            glTextureSubImage3D(_handle, level, 0, 0, 0, _extent.width, _extent.height, _extent.depth, _external_format, _external_type,
                                nullptr);
        else
        {
            glBindTexture(_type, _handle);
