...
{
    _queue.emplace_back(
        [ =, draw_mode = current_draw_mode(), et = _element_type.load(), eo = _element_offset.load(), es = _element_type_size.load() ] {
            glDrawElementsInstancedBaseVertexBaseInstance(draw_mode, index_count, et, reinterpret_cast<void*>(eo + base_index * es),
...
void commands_implementation::set_viewports(u32 first, gsl::span<viewport> vp, gsl::span<rect2f> scissors)
{
    assert(_curr_framebuffer);
    _queue.emplace_back([
        first, vps = std::vector<viewport>(vp.begin(), vp.end()), scs = std::vector<rect2f>(scissors.begin(), scissors.end()),
        h = _curr_framebuffer->height()
    ] {
