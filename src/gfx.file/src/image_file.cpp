...
#include "loaders/nanosvg.h"
#include "loaders/nanosvgrast.h"
        ...
        : file(svg)
{
    ...
    {
        void operator()(NSVGrasterizer* r) const { nsvgDeleteRasterizer(r); }
    };
    ...
    auto&& vec = std::get<std::vector<uint8_t>>(
            data = std::vector<uint8_t>(size_t(raster_scale * raster_scale * parsed->width * parsed->height * 4)));
    channels     = 4;
    ...
    nsvgRasterize(rasterizer.get(),
                  ...
                  0,
                  raster_scale,
                  static_cast<uint8_t*>(vec.data()),
                  static_cast<int>(raster_scale * parsed->width),
                  static_cast<int>(raster_scale * parsed->height),
                  static_cast<int>(raster_scale * parsed->width) * 4);
    nsvgDelete(parsed);

    _raw = vec.data();
}

int image_file::pixel_count() const noexcept { return width * height; }

void* image_file::bytes() const noexcept { return _raw; }
}