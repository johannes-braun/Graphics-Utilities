...
buffer<T>::buffer(device& d, mapped<T> const& source) : buffer(d)
{
    ...
    allocate(source.capacity(), false);
    _size = source.size();
...
template<typename T>
void buffer<T>::allocate(size_type capacity, bool force)
{
    if (force || _capacity < capacity)
    {
        _capacity = capacity;
        ...
        commands copy_cmd = _device->allocate_transfer_command();
        copy_cmd.get_command_buffer().begin({vk::CommandBufferUsageFlagBits::eOneTimeSubmit});
        if (_buffer) copy_cmd.get_command_buffer().copyBuffer(_buffer, nb, vk::BufferCopy(0, 0, _size));
        copy_cmd.get_command_buffer().end();
        _device->transfer_queue().submit({copy_cmd}, {}, {});
        _device->transfer_queue().wait();
