...
public:
    ...
    [[nodiscard]] auto images() const noexcept -> std::vector<image> const&;
    ...
    [[nodiscard]] auto    swap(opt_ref<semaphore const> sem = std::nullopt, opt_ref<fence const> fen = std::nullopt,
                               std::chrono::nanoseconds timeout = std::chrono::nanoseconds::max()) -> std::optional<acquire_error>;
    [[nodiscard]] auto    current_index() const noexcept -> uint32_t;
    [[nodiscard]] auto    current_image() const noexcept -> image const&;
