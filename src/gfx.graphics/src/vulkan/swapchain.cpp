...
std::vector<image> const& swapchain::images() const noexcept
{
    return _images;
}
...
std::optional<acquire_error> swapchain::swap(opt_ref<semaphore const> sem, opt_ref<fence const> fen, std::chrono::nanoseconds timeout)
{
    u32  img       = 0;
...
auto swapchain::current_image() const noexcept -> image const&
{
