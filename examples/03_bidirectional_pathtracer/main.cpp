...
int main()
{
    auto context = gfx::context::create(gfx::gapi::opengl, "[03] Bidirectional Pathtracer", 800, 600);
    ...
    cubemap.assign(0, 0, 0, w, h, 1, GL_RGB, GL_FLOAT, gfx::image_file("hdri/hdr/posx.hdr", gfx::bits::b32, 3).bytes());
    cubemap.assign(0, 0, 1, w, h, 1, GL_RGB, GL_FLOAT, gfx::image_file("hdri/hdr/negx.hdr", gfx::bits::b32, 3).bytes());
    cubemap.assign(0, 0, 2, w, h, 1, GL_RGB, GL_FLOAT, gfx::image_file("hdri/hdr/posy.hdr", gfx::bits::b32, 3).bytes());
    cubemap.assign(0, 0, 3, w, h, 1, GL_RGB, GL_FLOAT, gfx::image_file("hdri/hdr/negy.hdr", gfx::bits::b32, 3).bytes());
    cubemap.assign(0, 0, 4, w, h, 1, GL_RGB, GL_FLOAT, gfx::image_file("hdri/hdr/posz.hdr", gfx::bits::b32, 3).bytes());
    cubemap.assign(0, 0, 5, w, h, 1, GL_RGB, GL_FLOAT, gfx::image_file("hdri/hdr/negz.hdr", gfx::bits::b32, 3).bytes());
    ...
    while (context->run())
    {
        ...
        {
            if(const auto item = gfx::file::open_dialog("Open Mesh", "../", {"*.dae", "*.fbx", "*.obj", "*.stl", "*.ply", "*.blend"}, "Meshes"))
            {
        ...
        if (ImGui::Button("Save"))
        {
            if(const auto path = gfx::file::save_dialog("Save Render", "../", {"*.png"}, "PNG Files"))
            {
                ...
                gfx::image_file::save_png(path.value(), 800, 600, 4, u8data.data());
            }
        }
        ImGui::End();
        imgui.render();
