...
struct glblur_pass
{
    glblur_pass(glm::ivec2 size, std::uint32_t direction, std::uint32_t half_size) : size(size)
    {
    ...
    void render(mygl::texture input)
    {
        ...
        glUniform2iv(0, 1, glm::value_ptr(size));
        glUniform1ui(2, blur_direction);
...
private:
    void create_pipeline(std::uint32_t direction, std::uint32_t half_size)
    {
        ...
        std::uint32_t constants[]{half_size, direction};
        std::uint32_t indices[]{0u, 1u};
       ...
       // glSpecializeShader(frag, "main", std::size(constants), indices, constants);
