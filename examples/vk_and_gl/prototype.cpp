                                    ...
                                    std::optional<bounds3f> bounds)
{
    ...
    _index_buffer = std::make_unique<buffer<index32>>(_staging_index_buffer);
    if (_flags.has(mesh_allocator_flag::use_bvh)) _bvh_buffer = std::make_unique<buffer<bvh<3>::node>>(_staging_bvh_buffer);
    _vertex_buffer = std::make_unique<buffer<vertex3d>>(_staging_vertex_buffer);
...
void mesh_allocator::free_mesh(const mesh* m)
{
    ...
    if (_flags.has(mesh_allocator_flag::use_bvh))
    {
        ...
        _bvh_buffer = std::make_unique<buffer<bvh<3>::node>>(_staging_bvh_buffer);
...
{
    return unique_mesh(allocate_mesh(mesh, submesh), [this](gfx::gl::mesh* m) { free_mesh(m); });
}

shared_mesh mesh_allocator::allocate_mesh_shared(const mesh3d& mesh, const submesh3d& submesh)
{
    return shared_mesh(allocate_mesh(mesh, submesh), [this](gfx::gl::mesh* m) { free_mesh(m); });
}
