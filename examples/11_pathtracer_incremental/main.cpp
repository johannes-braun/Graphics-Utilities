...
void runnable::run()
{
	...
	gfx::hbuffer<gfx::index32> indices_staging = scene.meshes[0].indices;
	gfx::bvh<3> bvh(gfx::shape::triangle);
	bvh.sort(indices_staging.begin(), indices_staging.end(), [&](gfx::index32 i) { return scene.meshes[0].vertices[i].position; });
	const gfx::buffer<gfx::bvh<3>::node> model_bvh(gfx::buffer_usage::storage, bvh.nodes());
	const gfx::buffer<gfx::vertex3d> vertices(gfx::buffer_usage::storage, scene.meshes[0].vertices);
	const gfx::buffer<gfx::index32> indices(gfx::buffer_usage::storage, indices_staging);
