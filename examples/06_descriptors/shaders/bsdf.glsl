...
vec3 sample_bsdf(vec2 random_value) 
{
	...
	float alpha2 = max(instance_state.roughness * instance_state.roughness, 0.01f);
...
vec3 evaluate_bsdf(vec3 out_dir, vec2 random_value) 
{
	...
	float inner = _is_transmitted ? 1+0*DistributionGGX(_msurf_normal, -ray_state.direction, instance_state.roughness) :
		(max(dot(hit_state.normal, out_dir), 0) / (3.14159265359f));
	float outer = DistributionGGX(_msurf_normal, -ray_state.direction, instance_state.roughness);
	return _is_reflected ? mix(vec3(1.f), unpackUnorm4x8(instance_state.color).rgb, sqrt(instance_state.reflectivity)) :  inner*unpackUnorm4x8(instance_state.color).rgb;
}
float distribution_bsdf(vec3 out_dir, vec2 random_value) 
{
	float inner = _is_transmitted ? 1+0*DistributionGGX(_msurf_normal, -ray_state.direction, instance_state.roughness) :
