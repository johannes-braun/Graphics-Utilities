...
int main()
{
    ...
    gfx::host_buffer<glm::mat4> cubemap_infos(1);
    cubemap_infos[0] = glm::mat4(1.f);
    gfx::host_buffer<glm::ivec4> cubemap_layers(6);
    std::iota(cubemap_layers.begin(), cubemap_layers.end(), glm::ivec4(0));

    gfx::device_image cubemap_target(gfx::img_type::image2d, gfx::rgba16f, gfx::extent{256, 256, 6}, 1);
    gfx::device_image cubemap_depth(gfx::img_type::image2d, gfx::d32f, gfx::extent{256, 256, 6}, 1);
    ...
    glBindBufferBase(GL_UNIFORM_BUFFER, 1, cubemap_infos);

    cubemap_framebuffer.bind();
    cubemap_framebuffer.set_drawbuffer(GL_COLOR_ATTACHMENT0);
    cubemap_framebuffer.clear(0, {0.003f, 0.203f, 0.004f, 1.f});
    cubemap_framebuffer.clear(0.f, 0);
    ...
    for (int i = 0; i < 6; ++i) {
        glBindBufferRange(GL_SHADER_STORAGE_BUFFER, 2, cubemap_layers, 128, sizeof(glm::ivec4));
        small_meshes.render();
        large_meshes.render();
    }
