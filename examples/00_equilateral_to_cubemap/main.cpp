...
{
    const glm::vec3 side_dir = normalize(cross(look_dir, up));
...
int main(const int argc, char** argv)
{
    ...
    if(const auto dst = gfx::file::folder_dialog("Select Target Folder", "."))
    {
        dest = dst.value();
    }
    else
...
#pragma omp critical
            ...
            for(int i = 0; i < png.size(); ++i)
                png[i] = static_cast<glm::u8vec3>(glm::clamp(glm::pow(new_image[i], glm::vec3(1.f/1.75f)) * glm::vec3(255), glm::vec3(0), glm::vec3(255)));
