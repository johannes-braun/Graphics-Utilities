...
public:
    ...
    ui(GLFWwindow* w = nullptr)
            ...
            , _ibuf(GL_DYNAMIC_STORAGE_BIT | GL_MAP_READ_BIT | GL_MAP_WRITE_BIT |
                    GL_MAP_PERSISTENT_BIT)
            , _vbuf(GL_DYNAMIC_STORAGE_BIT | GL_MAP_READ_BIT | GL_MAP_WRITE_BIT |
                    GL_MAP_PERSISTENT_BIT)
    ...
    void draw()
    {
        ...
        _pbuf[0].screen =
                glm::scale(glm::translate(glm::vec3(-1, -1, 0)), glm::vec3(2.f / w, 2.f / h, 1));
    ...
    {
        indirect ind;
        ind.base_vertex = static_cast<uint32_t>(_vbuf.size());
        ind.base_index  = static_cast<uint32_t>(_ibuf.size());
...
bool run_splash()
{
    float mx = static_cast<float>(sqrt(std::min(3.0f * (glfwGetTime() - _splash_time), 1.0)));

    int       size = static_cast<int>(256 * mx);
    ...
    get_ui().draw_quad({w / 2 - size / 2, h / 2 - size / 2},
