...
const std::array<index, 6> quad_indices{
    0, 1, 2,
    0, 2, 3
};
...
struct stbi_freer { void operator()(uint8_t* d) { free(d); } };
...
gl_texture_t msaa_color_attachment;
gl_texture_t msaa_depth_attachment;
...
int main(int argc, const char** args)
{
    ...
    gl_buffer_t buffers[2]; glCreateBuffers(2, buffers);
    ...
    glNamedBufferStorage(buffers[vertex_buffer_index], vertex_buffer_size, quad_vertices.data(), GL_NONE);
    glNamedBufferStorage(buffers[index_buffer_index], index_buffer_size, quad_indices.data(), GL_NONE);
    ...
    gl_vertex_array_t vertex_array; glCreateVertexArrays(1, &vertex_array);
    ...
    glVertexArrayAttribFormat(vertex_array, attribute_position, glm::vec4::length(), GL_FLOAT, false, offsetof(vertex, position));
    ...
    glVertexArrayAttribFormat(vertex_array, attribute_uv, glm::vec2::length(), GL_FLOAT, false, offsetof(vertex, uv));
    glVertexArrayAttribBinding(vertex_array, attribute_uv, vertex_buffer_binding);
    // Assign (or bind) our buffers to the vertex array.
    glVertexArrayVertexBuffer(vertex_array, vertex_buffer_binding, buffers[vertex_buffer_index], 0, sizeof(vertex));
    ...
    gl_shader_program_t vertex_shader = glCreateShaderProgramv(GL_VERTEX_SHADER, 2, vertex_shader_sources);
    constexpr const char* fragment_shader_sources[2] = { shader_header, fragment_shader_code };
    gl_shader_program_t fragment_shader = glCreateShaderProgramv(GL_FRAGMENT_SHADER, 2, fragment_shader_sources);

    const auto check_shader = [](gl_shader_program_t program){
    ...
    gl_program_pipeline_t pipeline; glCreateProgramPipelines(1, &pipeline);
    ...
    gl_buffer_t matrix_buffer; glCreateBuffers(1, &matrix_buffer);
    // Make the buffer dynamic, so we can assign data to it after allocation, enable writing when mapped and leave the buffer data mapped if done so.
    glNamedBufferStorage(matrix_buffer, sizeof(matrices), nullptr, GL_DYNAMIC_STORAGE_BIT | GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT);
    matrices* mapped_matrix_buffer = static_cast<matrices*>(glMapNamedBuffer(matrix_buffer, GL_WRITE_ONLY));
    ...
    stbi_data texture_data(stbi_load("../res/brick.png", &texture_width, &texture_height, nullptr, STBI_rgb_alpha)); // put in your own path.
    ...
    gl_sampler_t sampler; glCreateSamplers(1, &sampler);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        mapped_matrix_buffer->view_projection =
            glm::perspectiveFov(glm::radians(60.f), static_cast<float>(w), static_cast<float>(h), 0.01f, 100.f) *   // projection
