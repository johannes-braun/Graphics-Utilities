...
constexpr vk::app_info               default_app_info{"My App", {1, 0}, "My Engine", {1, 0}};
...
int main()
{
    ...
    auto main_instance  = std::make_shared<vk::instance>(default_app_info, layers, final_instance_extensions);
    auto debug_callback = vk::create<vk::debug_callback>(
            main_instance,
    ...
    auto main_gpu = [&]() -> std::shared_ptr<vk::physical_device> {
        for(const std::shared_ptr<vk::physical_device>& phys_device : main_instance->get_physical_devices())
            return phys_device;
        return nullptr;
    }();
    auto surface = vk::create<vk::surface>(main_gpu, *window);

    std::vector<VkQueueFamilyProperties> properties = main_gpu->get_queue_family_properties();
    const auto graphics_queue_iter = std::find_if(properties.begin(), properties.end(), [](const VkQueueFamilyProperties& p) {
        return (p.queueFlags & VK_QUEUE_GRAPHICS_BIT) != 0;
    });
    const auto compute_queue_iter  = std::find_if(properties.begin(), properties.end(), [](const VkQueueFamilyProperties& p) {
        return (p.queueFlags & VK_QUEUE_COMPUTE_BIT) != 0;
    });
    const auto transfer_queue_iter = std::find_if(properties.begin(), properties.end(), [](const VkQueueFamilyProperties& p) {
        return (p.queueFlags & VK_QUEUE_TRANSFER_BIT) != 0;
    });
    ...
    queue_datas.push_back({uint32_t(graphics_queue_iter - properties.begin()),
                           uint32_t(unique_families[static_cast<uint32_t>(graphics_queue_iter - properties.begin())].size() - 1)});
    unique_families[static_cast<uint32_t>(compute_queue_iter - properties.begin())].push_back(queue_priorities[queue_index_compute]);
    queue_datas.push_back({uint32_t(compute_queue_iter - properties.begin()),
                           uint32_t(unique_families[static_cast<uint32_t>(compute_queue_iter - properties.begin())].size() - 1)});
    ...
    queue_datas.push_back({uint32_t(transfer_queue_iter - properties.begin()),
                           uint32_t(unique_families[static_cast<uint32_t>(transfer_queue_iter - properties.begin())].size() - 1)});
    unique_families[static_cast<uint32_t>(present_queue_iter - properties.begin())].push_back(queue_priorities[queue_index_present]);
    queue_datas.push_back({uint32_t(present_queue_iter - properties.begin()),
                           uint32_t(unique_families[static_cast<uint32_t>(present_queue_iter - properties.begin())].size() - 1)});

    std::vector<vk::queue_info> queue_infos;
    for(const auto& fam : unique_families)
        queue_infos.emplace_back(fam.first, fam.second);
    auto device = vk::create<vk::device>(main_gpu, layers, device_extensions, queue_infos, main_gpu->get_features());

    std::vector<vk::queue> queues;
    for(auto&& data : queue_datas)
        queues.emplace_back(device, data.family, data.index);
    ...
    auto       command_pool =
            vk::create<vk::command_pool>(device, queues[queue_index_graphics].family(), VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT);
    const auto command_buffer   = vk::create<vk::command_buffer>(command_pool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
    ...
    vk::subpass_info subpass(VK_PIPELINE_BIND_POINT_GRAPHICS, {}, vk::attachment_reference{0, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL});
    auto             dependencies = vk::subpass_dependencies()
                                .to(0,
                                    VK_DEPENDENCY_BY_REGION_BIT,
                                    VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                                    VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT)
                                .finish(VK_DEPENDENCY_BY_REGION_BIT);
    auto color_attachment = vk::attachment_description({}, VK_FORMAT_B8G8R8A8_UNORM, VK_SAMPLE_COUNT_1_BIT)
    ...
    auto renderpass = vk::create<vk::renderpass>(device, color_attachment, subpass, dependencies);

    std::vector<std::shared_ptr<vk::image_view>>  swapchain_views(images.size());
    std::vector<std::shared_ptr<vk::framebuffer>> swapchain_framebuffers(images.size());
#pragma omp parallel for
    for(int i = 0; i < images.size(); ++i)
    {
        swapchain_views[i]        = images[i]->create_view(VK_IMAGE_VIEW_TYPE_2D, VK_FORMAT_B8G8R8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT);
        swapchain_framebuffers[i] = vk::create<vk::framebuffer>(renderpass, 1280, 720, swapchain_views[i]);
    }

    vk::multi_block_allocator allocator(device);

    uint32_t buf_families[] = {queue_datas[queue_index_transfer].family, queue_datas[queue_index_graphics].family};
    auto     buffer         = vk::create<vk::proxy_buffer>(
            device, 256, 0, VK_SHARING_MODE_CONCURRENT, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, buf_families);
    buffer->bind_memory(allocator.allocate(buffer->requirements(), VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT));

    command_buffer->begin(VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);
    const auto barrier = buffer->memory_barrier(
            256, 0, 0, VK_ACCESS_TRANSFER_READ_BIT, queue_datas[queue_index_graphics].family, queue_datas[queue_index_transfer].family);
    ...
    {
    public:
        query_pool(const std::shared_ptr<vk::device>& device, VkQueryType type, uint32_t count, VkQueryPipelineStatisticFlags pipeline_statistic = 0)
                : _device(device)
        {
            ...
            vkCreateQueryPool(*_device, &query_pool_info, nullptr, &_query_pool);
        }
    ...
    query_pool timer_query_pool(device, VK_QUERY_TYPE_TIMESTAMP, 2);
    ...
    while(!close_window)
    {
        ...
        vkCmdResetQueryPool(*primary_buffers[image], timer_query_pool, 0, 2);
        vkCmdWriteTimestamp(*primary_buffers[image], VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, timer_query_pool, 0);
        primary_buffers[image]->begin_renderpass(
                *renderpass, *swapchain_framebuffers[image], {0, 0, 1280, 720}, VK_SUBPASS_CONTENTS_INLINE, {{glm::vec4(1, 0, 0, 1)}});

        primary_buffers[image]->end_renderpass();

        vkCmdWriteTimestamp(*primary_buffers[image], VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, timer_query_pool, 1);

        primary_buffers[image]->end();
        queues[queue_index_present].submit(
                primary_buffers[image], render_semaphore, swap_semaphore, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, fences[image]);
        VkResult result = queues[queue_index_present].present(render_semaphore, swapchain, image);

        uint64_t stamp[2]{0};
        vkGetQueryPoolResults(*device, timer_query_pool, 0, 2, 2 * sizeof(uint64_t), stamp, sizeof(uint64_t), VK_QUERY_RESULT_WAIT_BIT);
        uint64_t elapsed = stamp[1] - stamp[0];
