...
layout(set = 1, binding = 0, std430) readonly buffer Models { mesh_info meshes[]; };
struct material_info
{
    vec3 f0;
    uint color_roughness;
    uint textures_diffuse;
    uint pad[3];
};
layout(set = 1, binding = 1, std430) readonly buffer Materials { material_info materials[]; };

layout(set = 2, binding = 0) uniform sampler2DShadow shadow_maps[1];
layout(set = 1, binding = 2) uniform sampler2D textures[256];
...
layout(set = 2, binding = 1, std430) readonly buffer Lights { light_info lights[]; };
...
void main()
{
    ...
    for(int i = 0; i < 1; ++i)
    {
        ...
        if(cligh.shadow_map != -1)
        {
            ...
            shd *= 1 - shadow(shadow_maps[cligh.shadow_map],
                              shadow_matrix,
                              position,
                              normal,
                              light_dir);
        ...
        vec3  fresnel_v =
                material.f0 + (1 - material.f0) * pow(1 - max(dot(view_dir, normal), 0), 5);
        vec3 fresnel_l =
                material.f0 + (1 - material.f0) * pow(1 - max(dot(light_dir, normal), 0), 5);
